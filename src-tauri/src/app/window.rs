...
fn build_window_with_label(
    ...
    // GoViewPro always serves its bundled site assets through the local
    // tauri-plugin-localhost server (or the Tauri dev_url in
    // `--iterative-build` dev mode), overriding pake.json's url/url_type
    // dispatch entirely. Applies to the main window and multi-window clones
    // (Cmd+N) alike; popups opened via on_new_window keep the actually
    // clicked target URL (see open_requested_window).
    let url = match tauri_config.build.dev_url.clone() {
        Some(dev_url) => WebviewUrl::External(dev_url),
        None => WebviewUrl::External("http://localhost:9527".parse().unwrap()),
    };
