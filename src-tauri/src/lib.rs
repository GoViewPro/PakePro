            ...
            if let tauri::WindowEvent::CloseRequested { api, .. } = _event {
                if hide_on_close {
                    if _window.label() == "pake" {
                        ...
                        // Secondary multi-window instances are destroyed on close
                        // rather than hidden with the main window.
                        let _ = _window.destroy();
                    }
                } else {
                    // Fork product decision: fully quit instead of persisting in the
                    // dock/tray when hide_on_close is disabled.
                    std::process::exit(0);
                }
            }
        })
